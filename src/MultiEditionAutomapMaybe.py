...
# Fate - Gates of Dawn
#
# This source code is intended to provide an automap for the program: 
# Fate - Gates of Dawn
# Commodore Amiga version
# running on a Windows system
# under either of two possible Amiga Emulators:
# FS-UAE.exe or WinUAE.exe

# In particular, certain system- and emulator- specific assumptions
# are made when searching for the correct Windows process,
# and also in the algorithms which are used to discover
# relevant memory locations used within that process.

# This source code has been developed and tested 
# using 32-bit Python 3.7.3 IDLE
# on Windows 10 x64 software
# and an HP Envy with an 
# Intel Core i7 processor and an 
...
def getProcessID(imageName):
    ...
    for task in tasks:
        # the structure "m" will contain Windows process ID information for each task (e.g. process)
        m = re.match(b'(.*?)\\s+(\\d+)\\s+(\\w+)\\s+(\\w+)\\s+(.*?)\\s.*', task.encode())
        if m is not None:
            print(m.group(0).decode(),m.group(1).decode(),m.group(2).decode(),m.group(3).decode(),m.group(4).decode())
            # compare (all lower case, to be safe) process name with desired process name
            if m.group(1).decode().lower() == imageName.lower():
...
def getMemoryChunk(thePID, theMemoryAddress, theVariablePtr, theVariableSize):
    ...
    ReadProcessMemory(processHandle, theMemoryAddress, theVariablePtr, theVariableSize, byref(bytesRead))
...
def putMemoryChunk(thePID, theMemoryAddress, theVariablePtr, theVariableSize):
    ...
    processHandle = OpenProcess(PROCESS_ALL_ACCESS, False, thePID)
    WriteProcessMemory(processHandle, theMemoryAddress, theVariablePtr, theVariableSize, byref(bytesWritten))
    CloseHandle(processHandle)   
...
if __name__ == '__main__':
    ...
    # ----------- This code expects to find either fs-uae.exe or winuae.exe ---------------
    # ----------- in the list of Windows process names                      ---------------
    ...
    for task in tasks:
        ...
        # The structure "m" will contain Windows process ID information for each task (e.g. process)
        # Source: "Tasklist output" discussion at https://stackoverflow.com/questions/13525882/tasklist-output
        
        m = re.match(b'(.*?)\\s+(\\d+)\\s+(\\w+)\\s+(\\w+)\\s+(.*?)\\s.*', task.encode())
        
        if m is not None:
            
            # This next print command is for debug purposes, to understand how the task group is encoded.
            # It should remain commented unless debugging.
            
            # print(m.group(0).decode(),m.group(1).decode(),m.group(2).decode(),m.group(3).decode(),m.group(4).decode())

            # Compare process name with desired process names (after converting to lower case, just to be safe).
            if m.group(1).decode().lower() == "fs-uae.exe" or m.group(1).decode().lower() == "winuae.exe":

                # If they match, remember both the PID and how much memory the process is using.
                
                myPID=int(m.group(2).decode())

                # I had a foolish notion that any process in Windows 
                # would have its own internal memory map
                # that would extend from 0000 to a maximum address
                # that could be inferred from the process size.                
                # So, I got the process size from the task list.
                # Sadly, that notion was laughably false.
                # Therefore, the next line is commented
                # because the process size was unhelpful.
                
                # myPIDmemory=int(m.group(5).decode().replace(',', '')) # process size in KB

                # Now exit the loop, since the desired process information has been found.
                break
            
    # This next print command is for debug purposes, to observe the process information that has been remembered.
    # It should remain commented unless debugging.
    ...
    # ---------------------------------------------------------------------
    # Next, try to determine the application's memory address ranges
    # Source: Nicolas Verdier and AlessandroZ at
    # https://github.com/n1nj4sec/memorpy/blob/master/memorpy/WinProcess.py
    # ---------------------------------------------------------------------




    # Implement technical\programming\python\SearchProcessMemory here.

    
    # ------------------------------------------------------------------------------------
    # test if sliced bytearray defined as "Winwood"
    # equals sliced byte array defined byte by byte.
    # Neither should have the string null termination.
    # ------------------------------------------------------------------------------------
    ...
    print("Winwood",myTest[0:6] == b"Winwood"[0:6],myPID)

    # ---------------------------------------------------------------------------------
    # set up variables that will be fetched from the FS-UAE.exe or WinUAE.exe processes
    # ---------------------------------------------------------------------------------

    emulatorMemory = bytearray(1024+10)
    emulatorMemoryPtr = ctypes.c_char * len(emulatorMemory)
    emulatorMemoryPtr = emulatorMemoryPtr.from_buffer(emulatorMemory)
    emulatorMemorySize = len(emulatorMemory)

    # must think about how to handle seams
    # where string of interest crosses boundary
    # between two 1024 byte memory reads
    ...
    processHandle = OpenProcess(PROCESS_ALL_ACCESS, False, myPID)

    # for emulatorMemoryAddress in range(0,myPIDmemory,1):
    for emulatorMemoryAddress in range(0,300000,1):
        ReadProcessMemory(processHandle, emulatorMemoryAddress, emulatorMemoryPtr, emulatorMemorySize+10, byref(bytesRead))
        if bytesRead:            
            for x in range(0,1024,1):

                # print(hex(emulatorMemoryAddress),hex(x),hex(emulatorMemory[x]),chr(emulatorMemory[x]))
                
                    
                if emulatorMemory[x] == b"Barbarian"[0]:

                    print(hex(emulatorMemoryAddress),x,emulatorMemory[x:(x+6)])
                    CloseHandle(processHandle)   
                    sys.exit()
                

    print(emulatorMemory[x],emulatorMemory[x+1],emulatorMemory[x+2],emulatorMemory[x+3],emulatorMemory[x+4],emulatorMemory[x+5],emulatorMemory[x+6])
    print(hex(emulatorMemoryAddress),x,emulatorMemory[x:(x+6)])
    ...
    '''
    versionIndex will not be used in this edition
    ...
    '''
    versionIndex=0          # initially assume English V1.6 under FS-UAE
    '''
    ...
    if (partyLoc == bytearray("Winwood",'ascii')):
        versionIndex=0          # if here, it has found the English V1.6    
    else:

        versionIndex=1          # if not, test for the Deutsche V1.7
        partyLocAddress = memPartyLoc[versionIndex]
        ...
        if (partyLoc != bytearray('Winwood'.encode('ascii'))):
            ...
            sys.exit()          # if here, both tests failed
    ...
    # Now that the major variables have been set up,
    # it is necessary to search memory for the process ID (PID) of either WinUAE.exe or FS-UAE.exe
