...
#
# BEHAVIOR:
#   - Prefer B10_MAIN_v###.py files (workflow optimization)
...
def get_recent_files(directory):
    ...
    # Prefer B10_MAIN_v### versioned files
    fa_files = [
...
def load_modules():
    ...
    c = importlib.import_module("B08_CONSTANTS_v002")
    f = importlib.import_module("B09_FUNCTIONS_v002")
...
#   (Assumptions, edge cases, risks.)
# ------------------------------------------------------------
def load_dump_into_module(c, f, filename):
    ...
    for group_block in tests:
        ...
        if required:
            if not os.path.exists(required):
                ...
                failed += 1
                continue
            load_dump_into_module(c, f, required)
...
def main():
    ...
    try:
        ...
        c, f = load_modules()
        tests_ok, passed, failed, function_results = run_tests(c, f, tests)
