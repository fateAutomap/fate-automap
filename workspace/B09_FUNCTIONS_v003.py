...
def get_process_list():
    ...
    snapshot = c.CreateToolhelp32Snapshot(c.TH32CS_SNAPPROCESS, 0)
    if snapshot == -1:
        return processes

    try:
        entry = c.PROCESSENTRY32()
        entry.dwSize = ctypes.sizeof(c.PROCESSENTRY32)

        success = c.Process32First(snapshot, ctypes.byref(entry))

        while success:
            processes.append((entry.szExeFile, entry.th32ProcessID))  # IMPORTANT
            success = c.Process32Next(snapshot, ctypes.byref(entry))
...
#        anchored by submap_index (c.ADDR_SUBMAP_INDEX)
#
# 4. render_space
...
if __name__ == "__main__":
    log = open(os.path.join(c.BASE_DIR, "console.txt"), "w", buffering=1)
    sys.stdout = log
    sys.stderr = log
...
def next_index(prefix,ext):
    i=1
    while os.path.exists(os.path.join(c.BASE_DIR, f"{prefix}_{i:03d}.{ext}")):
        i+=1
    return i
...
def resolve_addresses(profile, match_address):
    ...
    return {
        ...
        "c.ADDR_SUBMAP_INDEX": profile["c.ADDR_SUBMAP_INDEX"] + offset
    }
...
def read_mem(process,addr,size):
    ...
    if c.memory_mode=="process":
        buffer=ctypes.create_string_buffer(size)
        bytesRead=ctypes.c_size_t()
        ctypes.windll.kernel32.ReadProcessMemory(process,ctypes.c_void_p(addr),buffer,size,ctypes.byref(bytesRead))
        return buffer.raw

    # dump mode
    offset=addr-c.DUMP_START

    if offset<0 or offset+size>len(c.dump_data):
        return b"\x00"*size

    return c.dump_data[offset:offset+size]
...
def write_mem(process,addr,data):
    ...
    if c.memory_mode=="process":
        ...
        ctypes.windll.kernel32.WriteProcessMemory(process,ctypes.c_void_p(addr),buf,len(data),ctypes.byref(count))
        return

    # dump mode (modify RAM buffer only)
    offset=addr-c.DUMP_START

    if offset<0 or offset+len(data)>len(c.dump_data):
        return

    c.dump_data[offset:offset+len(data)]=data
...
def find_bytes(process, pattern, start=c.DUMP_START, size=c.DUMP_SIZE, chunk_size=0x1000):
    matches = []
    pat_len = len(pattern)
    overlap = pat_len - 1
    addr = start
...
def read_coords(process):
    raw=read_mem(process, c.COORD_ADDR,12)
    x=int.from_bytes(raw[0:4],"big")
    y=int.from_bytes(raw[4:8],"big")
    return x,y

def write_coords(process,x,y):
    write_mem(process, c.COORD_ADDR,x.to_bytes(4,"big"))
    write_mem(process, c.COORD_ADDR+4,y.to_bytes(4,"big"))

def read_class_table(process):
    names=[]
    for i in range(c.CLASS_COUNT):
        addr=c.BASE_CLASS_TABLE+i*c.CLASS_RECORD_SIZE
        raw=read_mem(process,addr,c.CLASS_NAME_LEN)
...
def read_encounters(process):
    raw=read_mem(process,c.ENCOUNTER_ADDR,24*99)
    enc=[]
    idx = 0
...
def draw_sidebar(draw_target,font,x,y,process,map_name,offset,sx,sy):
    ...
    panel_x=w-c.PANEL_WIDTH
    pygame.draw.rect(draw_target,(30,30,30),(panel_x,0,c.PANEL_WIDTH,h))
    ...
    y0+=8
    line("MAP",(0,200,200))
    line(f"b Turn Beacon {'OFF' if c.beacon else 'ON'}")
    line(f"c Turn Center Party {'OFF' if c.follow_player else 'ON'}")
    line(f"g Turn Grid {'OFF' if c.show_grid else 'ON'}")
    ...
    line(f"F3 Turn Encounter x,y {'OFF' if c.dbg_enc_coords else 'ON'}")
    line(f"F4 Turn Crosshair {'OFF' if c.dbg_crosshair else 'ON'}")
    line(f"F5 Turn Hex {'OFF' if c.dbg_hex else 'ON'}")
    line(f"F6 Turn Tint {'OFF' if c.dbg_tint else 'ON'}")
    line("F7 Screenshot")
    line("F8 RAM dump")
    line(f"F9 Turn Inspector {'OFF' if c.dbg_inspector else 'ON'}")
...
# CRITICAL: Submap index anchors current viewport into global map.
                sub_index_full = int.from_bytes(read_mem(process, c.ADDR_SUBMAP_INDEX, 2), "big")
...
# MAP MODE SWITCH: '90' denotes wilderness; anything else treated as city.
            if map_name=="90":
                bx,by=cursor_tile(mx,my,offset,sx,sy,c.WILD_W,c.WILD_H)
                lx = bx - 40
                ly = by - 40

# CRITICAL: Submap index anchors current viewport into global map.
                sub_index = read_mem(process, c.ADDR_SUBMAP_INDEX, 1)[0]
                col = sub_index & 0x0F
                row = sub_index >> 4
                origin_x = col * 40
                origin_y = row * 40
                ...
                tx,ty=cursor_tile(mx,my,offset,sx,sy,c.CITY_W,c.CITY_H)
                line(f"tile: {tx},{ty}")
        except Exception as e:
