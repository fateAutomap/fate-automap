import B08_CONSTANTS_v002 as c
import B09_FUNCTIONS_v002 as f
import time
import os
import pygame
import ctypes
...
def main():
    ...
    screenshot_counter=f.next_index(c.VERSION+"_screenshot","png")
    dump_counter=f.next_index(c.VERSION+"_memdump","bin")



    pid=f.detect_emulator()

    if pid:
        process=ctypes.windll.kernel32.OpenProcess(c.PROCESS_ALL_ACCESS,False,pid)
        if not process:
    ...
    if not pid:
        ...
        bins=[fname for fname in os.listdir() if fname.lower().endswith(".bin")]
    ...
    for vid, profile in c.VERSION_PROFILES.items():
        ...
        matches = f.find_bytes(process, profile["pattern"])
        if matches:
    ...
    c.ADDR_CLASS_TABLE  = found_profile["ADDR_CLASS_TABLE"]  + global_offset
    c.ADDR_SUBMAP_INDEX = found_profile["ADDR_SUBMAP_INDEX"] + global_offset
    c.ADDR_MAP_ID       = found_profile["ADDR_MAP_ID"]       + global_offset
    c.ADDR_COORDS       = found_profile["ADDR_COORDS"]       + global_offset
    c.ADDR_PARTY_BASE   = found_profile["ADDR_PARTY_BASE"]   + global_offset
    c.ADDR_ENCOUNTERS   = found_profile["ADDR_ENCOUNTERS"]   + global_offset
    c.ADDR_MAP_BUFFER   = found_profile["ADDR_MAP_BUFFER"]   + global_offset

    # Print final resolved map
    print("--- Memory Map Binding Complete ---")
    print(f"c.ADDR_COORDS:       {hex(c.ADDR_COORDS)}")
    print(f"c.ADDR_MAP_BUFFER:   {hex(c.ADDR_MAP_BUFFER)}")
    print("-----------------------------------")

    c.MAP_ADDR=c.ADDR_MAP_BUFFER
    c.MAP_ID_ADDR=c.ADDR_MAP_ID
    c.COORD_ADDR=c.ADDR_COORDS
    c.ENCOUNTER_ADDR=c.ADDR_ENCOUNTERS
    c.BASE_CLASS_TABLE=c.ADDR_CLASS_TABLE

    class_names=f.read_class_table(process)

    # --- determine desired window position ---
    if os.path.exists(c.WINDOW_POS_FILE):
        try:
            x,y = map(int,open(c.WINDOW_POS_FILE).read().split(","))
        except:
            x,y = 100,100
    else:
        x,y = 100,100
    ...
    pygame.display.set_caption(c.VERSION)
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if c.view_mode=="encounter" and event.type==pygame.MOUSEBUTTONDOWN:
                mx, my = event.pos
                draw_x = 0
                for key, _, width in c.ENC_COLS:
                    if draw_x <= mx <= draw_x + width:
                        c.enc_sort_key = key
                        # Re-sort local data to maintain synchronization with draw call
                        if c.enc_sort_key == "index": current_rows.sort(key=lambda r: r["index"])
                        elif c.enc_sort_key == "x": current_rows.sort(key=lambda r: r["x"])
                        elif c.enc_sort_key == "y": current_rows.sort(key=lambda r: r["y"])
                        elif c.enc_sort_key == "attitude": current_rows.sort(key=lambda r: r["attitude"])
                        elif c.enc_sort_key == "dist": current_rows.sort(key=lambda r: r["dist"])
                        elif c.enc_sort_key.startswith("q"): current_rows.sort(key=lambda r: r[c.enc_sort_key])
                        elif c.enc_sort_key.startswith("t"): current_rows.sort(key=lambda r: r[c.enc_sort_key] or "")
                        break
                    draw_x += width
            ...
            if event.type==pygame.KEYDOWN:
                ...
                if event.key==pygame.K_F2:
                    c.view_mode = "encounter" if c.view_mode=="map" else "map"

                if event.key==pygame.K_F3: c.dbg_enc_coords=not c.dbg_enc_coords
                if event.key==pygame.K_F12:
                    if pid:
                        focus_emulator(pid)
                if event.key==pygame.K_F4:
                    c.dbg_crosshair=not c.dbg_crosshair
                    pygame.mouse.set_visible(not c.dbg_crosshair)
                if event.key==pygame.K_F5: c.dbg_hex=not c.dbg_hex
                if event.key==pygame.K_F6: c.dbg_tint=not c.dbg_tint
                if event.key==pygame.K_F9: c.dbg_inspector=not c.dbg_inspector

                if c.view_mode=="encounter":
                    if event.key in (pygame.K_UP, pygame.K_KP8): c.enc_scroll=max(0,c.enc_scroll-1)
                    if event.key in (pygame.K_DOWN, pygame.K_KP2): c.enc_scroll+=1
                    if event.key in (pygame.K_PAGEUP, pygame.K_KP9): c.enc_scroll=max(0,c.enc_scroll-10)
                    if event.key in (pygame.K_PAGEDOWN, pygame.K_KP3): c.enc_scroll+=10

                if event.key==pygame.K_g: c.show_grid=not c.show_grid

                if event.key==pygame.K_c:
                    c.follow_player = not c.follow_player

                if event.key==pygame.K_b:
                    c.beacon = not c.beacon

                # maximize health
                if event.key==pygame.K_h:
                    for slot in range(28):
                        base = c.ADDR_PARTY_BASE + slot * c.CHAR_SIZE + c.CHAR_H
                        raw = f.read_mem(process, base, 4)
                        maxhp = raw[2:4]
                ...
                if event.key==pygame.K_m:
                    for slot in range(28):
                        base = c.ADDR_PARTY_BASE + slot * c.CHAR_SIZE + c.CHAR_M
                        raw = f.read_mem(process, base, 4)
                        maxmp = raw[2:4]
                ...
                if event.key==pygame.K_f:
                    for slot in range(28):
                        base = c.ADDR_PARTY_BASE + slot * c.CHAR_SIZE + c.CHAR_F
                        write_mem(process, base, b"\x00\x00")

                # zeroize thirst
                if event.key==pygame.K_d:
                    for slot in range(28):
                        base = c.ADDR_PARTY_BASE + slot * c.CHAR_SIZE + c.CHAR_D
                        write_mem(process, base, b"\x00\x00")

                # add 1000 piasters
                if event.key==pygame.K_p:
                    for slot in range(28):
                        base = c.ADDR_PARTY_BASE + slot * c.CHAR_SIZE + c.CHAR_P
                        raw = f.read_mem(process, base, 4)
...
# MAP MODE SWITCH: '90' denotes wilderness; anything else treated as city.
                    if map_name=="90":
                        cx,cy=f.cursor_tile(mx,my,offset,sx,sy,c.WILD_W,c.WILD_H)
# CRITICAL: Submap index anchors current viewport into global map.
                        sub_index_full = int.from_bytes(f.read_mem(process, c.ADDR_SUBMAP_INDEX, 2), "big")
# NOTE: sub_index stored in wider field; mask ensures only 8-bit index is used.
                        ...
                        cx,cy=f.cursor_tile(mx,my,offset,sx,sy,c.CITY_W,c.CITY_H)
                    write_coords(process,cx,cy)

                # screenshot
                if event.key==pygame.K_F7:
                    name=f"{c.VERSION}_screenshot_{screenshot_counter:03d}.png"
                    pygame.image.save(screen, os.path.join(c.BASE_DIR, name))
                    screenshot_counter+=1

                # game memory dump from emulator
                if event.key==pygame.K_F8:
                    data=f.read_mem(process,DUMP_START,DUMP_SIZE)
                    base=f"{c.VERSION}_memdump_{dump_counter:03d}"
                    open(os.path.join(c.BASE_DIR, base+".bin"),"wb").write(data)
                    open(os.path.join(c.BASE_DIR, base+".txt"),"w").write(data.hex())
                    dump_counter+=1

            if event.type==pygame.MOUSEWHEEL:
                if c.view_mode=="encounter":
                    c.enc_scroll=max(0,c.enc_scroll-event.y)
                    continue
                mx,my=pygame.mouse.get_pos()

# MAP MODE SWITCH: '90' denotes wilderness; anything else treated as city.
                if map_name=="90":
                    bx,by=f.cursor_tile(mx,my,offset,sx,sy,c.WILD_W,c.WILD_H)
# CRITICAL: Submap index anchors current viewport into global map.
                    sub_index_full = int.from_bytes(f.read_mem(process, c.ADDR_SUBMAP_INDEX, 2), "big")
# NOTE: sub_index stored in wider field; mask ensures only 8-bit index is used.
                    sub_index = sub_index_full & 0xFF
                    ...
                    wx, wy = f.cursor_tile(mx,my,offset,sx,sy,c.CITY_W,c.CITY_H)

                zoom*=1.1 if event.y>0 else 0.9

# MAP MODE SWITCH: '90' denotes wilderness; anything else treated as city.
                if map_name=="90":
                    sx=(map_w/c.WILD_W)*zoom
                    sy=(win_h/c.WILD_H)*zoom
                    bx, by = tile_to_buffer(wx, wy, sub_index)
                    px = offset[0] + (bx+0.5)*sx
                    py = offset[1] + (c.WILD_H-by-0.5)*sy
                else:
                    sx=(map_w/c.CITY_W)*zoom
                    sy=(win_h/c.CITY_H)*zoom
                    px = offset[0] + (wx+0.5)*sx
                    py = offset[1] + (c.CITY_H-wy-0.5)*sy
...
# NOTE: sub_index stored in wider field; mask ensures only 8-bit index is used.
        ...
        if time.time()-last_update>0.5:
            encounters = f.read_encounters(process)
            class_names = f.read_class_table(process)
            last_update = time.time()

        if c.view_mode=="encounter":
            cur_x,cur_y=f.read_coords(process)
            c.enc_scroll, current_rows = draw_encounter_screen(
                draw_target, font, encounters, class_names, c.enc_scroll, cur_x, cur_y
            )
        ...
        map_name=f.read_mem(process, c.MAP_ID_ADDR, 2).decode(errors="ignore")

        win_w,win_h=screen.get_size()
        map_w=win_w-c.PANEL_WIDTH

# MAP MODE SWITCH: '90' denotes wilderness; anything else treated as city.
        if map_name=="90":
            raw=f.read_mem(process, c.MAP_ADDR, c.WILD_STRIDE*c.WILD_H)
            sx=(map_w/c.WILD_W)*zoom
            sy=(win_h/c.WILD_H)*zoom
            ...
            for y in range(c.WILD_H):
                for xb in range(c.WILD_STRIDE):
                    byte=raw[y*c.WILD_STRIDE+xb]
                    hi=(byte>>4)&0xF
                    lo=byte&0xF
                    ...
                    for i,val in enumerate((hi,lo)):
                        ...
                        cy=int(offset[1]+(c.WILD_H-y-1)*sy)
                        r=pygame.Rect(cx,cy,int(sx+0.5)+1,int(sy+0.5)+1)
                        pygame.draw.rect(draw_target,c.NIBBLE_PALETTE.get(val,(0,0,0)),r)

                        if c.dbg_tint:
                            base = c.NIBBLE_PALETTE.get(val,(0,0,0))
                            scaled = (int(base[0] * c.TINT_BRIGHTNESS), int(base[1] * c.TINT_BRIGHTNESS), int(base[2] * c.TINT_BRIGHTNESS))
...
# NOTE: sub_index stored in wider field; mask ensures only 8-bit index is used.
                ...
                for wy in range(0, 400, 5):
                    ...
                    if 0 <= by < 120:
                        ...
                        if wy % 10 == 0:
                            ...
                            draw_target.blit(label, (offset[0] + c.WILD_W * sx + 5, py - label.get_height()//2))

        else:
            raw=f.read_mem(process, c.MAP_ADDR, c.CITY_W*c.CITY_H)
            sx=(map_w/c.CITY_W)*zoom
            sy=(win_h/c.CITY_H)*zoom

            tile_px = int(min(sx, sy))
            font_size = max(6, tile_px - 2)
            if font_size not in c.hex_font_cache:
                c.hex_font_cache[font_size] = pygame.font.SysFont("Courier", font_size)
            hex_font = c.hex_font_cache[font_size]

            for i,val in enumerate(raw):
                mx=i%c.CITY_W
                my=i//c.CITY_W
                cx=int(offset[0]+mx*sx)
                cy=int(offset[1]+(c.CITY_H-my-1)*sy)
            ...
            if c.show_grid:
                ...
                for tx in range(0, c.CITY_W, 5):
                    px = int(offset[0] + tx * sx)
                    color = (120, 120, 120) if tx % 10 == 0 else (60, 60, 60)
                    pygame.draw.line(draw_target, color, (px, offset[1]), (px, offset[1] + c.CITY_H * sy))
                    if tx % 10 == 0:
                        lbl = font.render(str(tx), True, (200, 200, 200))
                        draw_target.blit(lbl, (px - lbl.get_width()//2, offset[1] - 18))
                        draw_target.blit(lbl, (px - lbl.get_width()//2, offset[1] + c.CITY_H * sy + 4))

                # Horizontal lines & labels
                for ty in range(0, c.CITY_H, 5):
                    py = int(offset[1] + (c.CITY_H - ty) * sy)
                    color = (120, 120, 120) if ty % 10 == 0 else (60, 60, 60)
                    pygame.draw.line(draw_target, color, (offset[0], py), (offset[0] + c.CITY_W * sx, py))
                    if ty % 10 == 0:
                        lbl = font.render(str(ty), True, (200, 200, 200))
                        draw_target.blit(lbl, (offset[0] - lbl.get_width() - 5, py - lbl.get_height()//2))
                        draw_target.blit(lbl, (offset[0] + c.CITY_W * sx + 5, py - lbl.get_height()//2))
...
# NOTE: sub_index stored in wider field; mask ensures only 8-bit index is used.
            ...
            py=int(offset[1]+(c.WILD_H-by-0.5)*sy)
        else:
            sx=(map_w/c.CITY_W)*zoom
            sy=(win_h/c.CITY_H)*zoom
            px=int(offset[0]+(cur_x+0.5)*sx)
            py=int(offset[1]+(c.CITY_H-cur_y-0.5)*sy)
...
# NOTE: sub_index stored in wider field; mask ensures only 8-bit index is used.
                ...
                offset[1] = (win_h/2) - (c.WILD_H-by-0.5)*sy
            else:
                offset[0] = (map_w/2) - (cur_x+0.5)*sx
                offset[1] = (win_h/2) - (c.CITY_H-cur_y-0.5)*sy
        if c.beacon:
            bpx, bpy = px, py
            t = time.time()
...
# INVARIANT: Wilderness Y bounds [0,399]; prevents invalid rendering/teleport.
                ...
                sxp, syp = int(offset[0]+(ex_c+0.5)*sx), int(offset[1]+(c.WILD_H-ey_c-0.5)*sy)
            else:
                # City scaling logic: Restored c.CITY_H basis and non-wilderness sx/sy
                # v248: prune invalid city encounters outside 56x56 bounds
                if ex > 55 or ey > 55:
                    continue
                sxp, syp = int(offset[0]+(ex+0.5)*sx), int(offset[1]+(c.CITY_H-ey-0.5)*sy)
            
            color=f.encounter_color(att)
...
#            if c.dbg_enc_coords: name=f"{name}({ex},{ey}){class_id}"
            if c.dbg_enc_coords: name=f"{idx}({ex},{ey})"
            if name:
...
# RISK: Can introduce visual jitter depending on timing/frame sync.
        ...
        mx,my=pygame.mouse.get_pos()
# MAP MODE SWITCH: '90' denotes wilderness; anything else treated as city.
        if map_name=="90": cx,cy=f.cursor_tile(mx,my,offset,sx,sy,c.WILD_W,c.WILD_H)
        else: cx,cy=f.cursor_tile(mx,my,offset,sx,sy,c.CITY_W,c.CITY_H)
        ...
        f.draw_sidebar(draw_target,font,cur_x,cur_y,process,map_name,offset,sx,sy)
    ...
    try:
        ...
        open(c.WINDOW_POS_FILE,"w").write(f"{rect.left},{rect.top}")
    except: pass
    pygame.quit()
